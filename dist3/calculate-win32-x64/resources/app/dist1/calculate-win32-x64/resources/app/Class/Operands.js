//操作数类
const {creatRandom,gcd} = require('D://Desktop//calculate//Utils//index')
exports.Operands=class Operands{
        ...
        toStr(){
            ...
            if(molecular===0){ return `${integer}` }
                ...
                let convention = gcd(molecular,absDenominator)
                console.log(convention)
                console.log(molecular)
                console.log(absDenominator)
                console.log(molecular/convention)
                return (integer===0?`${molecular/convention}/${absDenominator/convention}`:`${integer}'${molecular/convention}/${absDenominator/convention}`) //整数为零，返回分数部分，整数不为0，返回整个假分数
            }
